...
function _hasOptions(match){
    ...
    return lastCommaPosition > match.indexOf(']');
}
...
function _getDataFromMatch(match){
    ...
    if(_hasOptions(match)){
        try {
            globs = _getGlobsFromMatch(match, match.indexOf(',') + 1, lastComma);
        }
        catch(e){
        ...
        try {
            options = JSON.parse(match.slice(lastComma + 1, match.length).trim());
        }
        catch(e){
